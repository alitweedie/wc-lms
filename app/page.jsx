...
const ROUNDS = [
  ...
  },
  {
    ...
    deadline:"Sun 28 Jun, 20:00 BST",
    note:"32 teams. No draws — extra time & penalties if level after 90 mins.",
    fixtures:[
      ["South Africa","Canada","Sun 28 Jun","20:00 BST"],
      ["Brazil","Japan","Mon 29 Jun","18:00"],
      ["Germany","Paraguay","Mon 29 Jun","21:30"],
      ["Netherlands","Morocco","Tue 30 Jun","02:00"],
      ["Ivory Coast","Norway","Tue 30 Jun","18:00"],
      ["France","Sweden","Tue 30 Jun","22:00"],
      ["Mexico","Ecuador","Wed 1 Jul","02:00"],
      ["England","DR Congo","Wed 1 Jul","17:00"],
      ["Belgium","Senegal","Wed 1 Jul","21:00"],
      ["USA","Bosnia & Herz.","Thu 2 Jul","01:00"],
      ["Spain","Austria","Thu 2 Jul","20:00"],
      ["Portugal","Croatia","Fri 3 Jul","00:00"],
      ["Switzerland","Algeria","Fri 3 Jul","04:00"],
      ["Australia","Egypt","Fri 3 Jul","19:00"],
      ["Argentina","Cape Verde","Fri 3 Jul","23:00"],
      ["Colombia","Ghana","Sat 4 Jul","02:30"],
    ],
  },
  {
    id:5, label:"Round of 16", stage:"Round of 16",
    deadline:"Sat 4 Jul, 18:00 BST",
    note:"16 teams remaining. No draws.",
    fixtures:[
      ["R32 M73 Winner","R32 M75 Winner","Sat 4 Jul","18:00 BST"],
      ["R32 M74 Winner","R32 M77 Winner","Sat 4 Jul","22:00"],
      ["R32 M76 Winner","R32 M78 Winner","Sun 5 Jul","21:00"],
      ["R32 M79 Winner","R32 M80 Winner","Mon 6 Jul","01:00"],
      ["R32 M83 Winner","R32 M84 Winner","Mon 6 Jul","20:00"],
      ["R32 M81 Winner","R32 M82 Winner","Tue 7 Jul","01:00"],
      ["R32 M86 Winner","R32 M88 Winner","Tue 7 Jul","17:00"],
      ["R32 M85 Winner","R32 M87 Winner","Tue 7 Jul","21:00"],
    ],
  },
  {
    ...
    fixtures:[
      ["R16 M89 Winner","R16 M90 Winner","Thu 9 Jul","21:00 BST"],
      ["R16 M93 Winner","R16 M94 Winner","Fri 10 Jul","20:00"],
      ["R16 M91 Winner","R16 M92 Winner","Sat 11 Jul","22:00"],
      ["R16 M95 Winner","R16 M96 Winner","Sun 12 Jul","02:00"],
    ],
  },
  {
    ...
    fixtures:[
      ["QF M97 Winner","QF M98 Winner","Tue 14 Jul","20:00 BST"],
      ["QF M99 Winner","QF M100 Winner","Wed 15 Jul","20:00"],
    ],
  },
  {
    id:8, label:"Final", stage:"Final",
    deadline:"Sun 19 Jul, 20:00 BST",
    note:"The World Cup Final at MetLife Stadium, New Jersey. Tiebreaker: predict the minute of the first goal.",
    fixtures:[["SF M101 Winner","SF M102 Winner","Sun 19 Jul","20:00 BST"]],
    hasTiebreaker: true,
  },
];
...
function RoundSummaryButton({ round, wcRound, gi, survivors, ousted, aliveAtStart, matchResults, fixtureOverrides, setRoundSummary, adminMode }) {
  ...
  function resultForTeam(team) {
    ...
    const forGoals = isHome ? rec.h : rec.a;   // in-play goals (excl. shoot-out)
    const agGoals  = isHome ? rec.a : rec.h;
    // Penalty shoot-out: the match was drawn in play, then decided on pens.
    if (rec.dur === "PENALTY_SHOOTOUT" && rec.ph!=null && rec.pa!=null) {
      const pf = isHome ? rec.ph : rec.pa, pa = isHome ? rec.pa : rec.ph;
      const won = pf > pa;
      return `${team} drew ${forGoals}-${agGoals} with ${opp} after extra time, then ${won?"won":"lost"} the penalty shoot-out ${pf}-${pa}`;
    }
    const etNote = rec.dur === "EXTRA_TIME" ? " after extra time" : "";
    const verb = forGoals>agGoals ? "beat" : forGoals<agGoals ? "lost to" : "drew with";
    return `${team} ${verb} ${opp} ${forGoals}-${agGoals}${etNote}`;
  }
...
function FixturesTab({ matchResults, fixtureOverrides }) {
  ...
  // Keep the open round tracking the current live round as results come in,
  // until the user manually expands/collapses something — then respect their choice.
  useEffect(() => {
    if (!userTapped.current) setOpenRound(defaultRoundId);
  }, [defaultRoundId]);
  const onToggleRound = (id) => { userTapped.current = true; setOpenRound(cur => cur===id ? null : id); };
  return (
      ...
      {ROUNDS.map(wcRound=>{
        ...
        return (
          <div key={round.id} style={S.fixtureSection}>
            <div style={S.fixtureSectionHeader} onClick={()=>onToggleRound(round.id)}>
            ...
            {isOpen&&(
              <div style={{width:"100%"}}>
                {fixtures.map(([home,away,date,time],i)=>{
                  const result=matchResults[`${round.id}|${home}|${away}`];
                  const hasScore=result!==undefined;
                  const homeWon=hasScore&&result.h>result.a;
                  const awayWon=hasScore&&result.a>result.h;
                  return (
                    <div key={i} style={{display:"flex",alignItems:"center",padding:"9px 14px",borderBottom:"1px solid #111318",boxSizing:"border-box",width:"100%"}}>
                      <span style={{width:52,flexShrink:0,fontSize:9,color:"#666",lineHeight:1.4}}><span style={{display:"block"}}>{date}</span><span style={{display:"block"}}>{time.replace(" BST","")}</span></span>
                      <span style={{flex:1,minWidth:0,textAlign:"right",fontSize:12,color:hasScore?(homeWon?"#a8e031":"#888"):"#ddd",fontWeight:homeWon?700:400,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{home} {FLAG[home]||"🏳️"}</span>
                      <span style={{width:48,flexShrink:0,textAlign:"center",fontSize:hasScore?13:10,color:hasScore?"#fff":"#555",fontFamily:hasScore?"'Bebas Neue',sans-serif":"inherit",letterSpacing:hasScore?1:0}}>{hasScore?`${result.h}-${result.a}`:"v"}</span>
                      <span style={{flex:1,minWidth:0,textAlign:"left",fontSize:12,color:hasScore?(awayWon?"#a8e031":"#888"):"#ddd",fontWeight:awayWon?700:400,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{FLAG[away]||"🏳️"} {away}</span>
                    </div>
                  );
                })}
              </div>
...
const S = {
  ...
  main:{padding:"14px 14px",maxWidth:900,width:"100%",margin:"0 auto",flex:1,overflowY:"auto",boxSizing:"border-box",WebkitOverflowScrolling:"touch"},
